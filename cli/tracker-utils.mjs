 ...
 * `applications.md` (original root layout). The `GET_YOURSELF_TRACKER` env var
 ...
 * on one spelling.
 *
 * @param {string} rootDir - The get-yourself-cli repository root.
 ...
 * that pointing `GET_YOURSELF_TRACKER` at another workspace moves the whole set
 ...
 * a tracker. `GET_YOURSELF_PDF_INDEX` overrides it explicitly.
 *
 ...
 * that resolve the tracker from `GET_YOURSELF_TRACKER` then read one workspace's
...
export function resolvePdfIndexPath(trackerPath) {
  return process.env.GET_YOURSELF_PDF_INDEX
 ...
 * contends on the same lock. `GET_YOURSELF_TRACKER_LOCK` exists for tests and
 ...
 * use the get-yourself-cli lock-name prefix. Invalid values are ignored and the
...
export function trackerLockDirFor(appsFile) {
  ...
  const fallback = join(tmpRoot, `get-yourself-cli-merge-tracker-${lockKey}.lock`);
  const envValue = process.env.GET_YOURSELF_TRACKER_LOCK;
  ...
  if (!basename(candidate).startsWith('get-yourself-cli-merge-tracker-')) return fallback;
  return candidate;
}
