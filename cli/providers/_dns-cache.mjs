 ...
 * becomes 1.
 *
 * Why patch `dns.lookup` rather than configure the HTTP client: get-yourself-cli
 ...
 *   - `GET_YOURSELF_NO_DNS_CACHE=1` opts out entirely — no memoization AND no
 *     pacing, since both live inside this patched lookup.
 *   - `GET_YOURSELF_DNS_LOOKUPS_PER_MIN` caps resolver-bound lookups
...
export function lookupsPerMinFromEnv(env = process.env) {
  const raw = env.GET_YOURSELF_DNS_LOOKUPS_PER_MIN;
  ...
  if (!Number.isFinite(n) || n < 0) {
    console.error(
      `⚠ GET_YOURSELF_DNS_LOOKUPS_PER_MIN=${raw} is not a non-negative number — `
...
if (process.env.GET_YOURSELF_NO_DNS_CACHE !== '1') {
