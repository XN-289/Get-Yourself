 ...
 *
 * Run: node get-yourself-cli/merge-tracker.mjs [--dry-run] [--verify]
 */
...
const GET_YOURSELF = getCareerOpsRoot();
// Support both layouts: data/applications.md (boilerplate) and applications.md
// (original). GET_YOURSELF_TRACKER overrides the path (used by tests and
...
const APPS_FILE = resolveTrackerPath(GET_YOURSELF);
const TRACKER_DIR = dirname(APPS_FILE);
// GET_YOURSELF_ADDITIONS overrides the additions dir (used by tests, mirrors GET_YOURSELF_TRACKER).
const ADDITIONS_DIR = process.env.GET_YOURSELF_ADDITIONS
  ? process.env.GET_YOURSELF_ADDITIONS
  : join(GET_YOURSELF, 'batch/tracker-additions');
const MERGED_DIR = join(ADDITIONS_DIR, 'merged');
// GET_YOURSELF_BATCH_STATE overrides the batch-state.tsv path (used by tests).
const BATCH_STATE_FILE = process.env.GET_YOURSELF_BATCH_STATE
  ? process.env.GET_YOURSELF_BATCH_STATE
  : join(GET_YOURSELF, 'batch/batch-state.tsv');
...
const MERGE_HOLD_MS = Number(process.env.GET_YOURSELF_MERGE_HOLD_MS) || 0;
const MERGE_READY_IPC = process.env.GET_YOURSELF_MERGE_READY_IPC === '1';
...
mkdirSync(join(GET_YOURSELF, 'data'), { recursive: true });
mkdirSync(ADDITIONS_DIR, { recursive: true });

/**
 * Pause the async merge flow for a fixed number of milliseconds.
 *
 * Used by the regression test hook (`GET_YOURSELF_MERGE_HOLD_MS`), which
...
try {
  trackerLock = await acquireTrackerLock(TRACKER_LOCK_DIR, {
    timeoutMs: Number(process.env.GET_YOURSELF_TRACKER_LOCK_TIMEOUT_MS) || 60_000,
    retryMs: Number(process.env.GET_YOURSELF_TRACKER_LOCK_RETRY_MS) || 75,
    staleMs: Number(process.env.GET_YOURSELF_TRACKER_LOCK_STALE_MS) || 10 * 60_000,
    tracker: APPS_FILE,
  });
...
// so a tracker carrying columns get-yourself-cli has no field for (Apply Link,
...
// get-yourself-cli knows about is placed at ITS OWN detected index, and any column
// the header declares but get-yourself-cli has no value for becomes '—'.
//
...
function buildRow(o) {
  ...
  // start from the row's current cells so values in columns get-yourself-cli has no
  ...
  // overwrite only the fields get-yourself-cli owns. New rows pass no `raw` and keep
  // the plain '—' fill.
  if (o.raw) {
...
if (MIGRATE) {
  ...
  if (DRY_RUN) {
    ...
    console.log(`✅ Migration: rewrote ${changed} report link(s) in ${basename(APPS_FILE)} relative to ${TRACKER_DIR === GET_YOURSELF ? 'repo root' : 'data/'}`);
  }
  process.exit(0);
}
...
if (HEADER_WIDTH != null && HEADER_WIDTH - 2 > Object.keys(COLMAP).length) {
  console.log(
    ...
    'not mapped to a get-yourself-cli field — those cells are written as "—".',
  );
}
const existingApps = [];
let maxNum = 0;
...
if (!DRY_RUN) {
  try {
    execFileSync('node', [join(GET_YOURSELF, 'sync-pdf-flags.mjs')], { stdio: 'inherit' });
  } catch (e) {
...
if (VERIFY && !DRY_RUN) {
  ...
  try {
    execFileSync('node', [join(GET_YOURSELF, 'verify-pipeline.mjs')], { stdio: 'inherit' });
  } catch (e) {
    process.exit(1);
  }
}
