#!/usr/bin/env node
/**
 * add-entry.mjs — Deterministic dedup + insertion for `/get-yourself-cli add`.
 *
 ...
 *
 * Test isolation: GET_YOURSELF_CV / GET_YOURSELF_ARTICLE_DIGEST override the target
...
const CV_FILE = process.env.GET_YOURSELF_CV || join(GET_YOURSELF, 'cv.md');
const ARTICLE_DIGEST_FILE = process.env.GET_YOURSELF_ARTICLE_DIGEST || join(GET_YOURSELF, 'article-digest.md');
...
export function applyAdd(payload, { cvText = null, articleText = null } = {}) {
  ...
  if (payload.articleDigest) {
    ...
    const current = articleText === null
      ? '# Article Digest -- Proof Points\n\nCompact proof points from portfolio projects. Read by get-yourself-cli at evaluation time.\n'
      : articleText;
