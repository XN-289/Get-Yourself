...
// overlapping a manual `/get-yourself-cli pipeline` run, or two plugin jobs) can
...
export async function acquirePipelineLock(pipelinePath, options = {}) {
  ...
  const timeoutMs = options.timeoutMs ?? (Number(process.env.GET_YOURSELF_PIPELINE_LOCK_TIMEOUT_MS) || DEFAULT_TIMEOUT_MS);
  const retryMs = options.retryMs ?? (Number(process.env.GET_YOURSELF_PIPELINE_LOCK_RETRY_MS) || DEFAULT_RETRY_MS);
  const staleMs = options.staleMs ?? (Number(process.env.GET_YOURSELF_PIPELINE_LOCK_STALE_MS) || DEFAULT_STALE_MS);
  ...
  const envMaxWait = process.env.GET_YOURSELF_PIPELINE_LOCK_MAX_WAIT_MS;
