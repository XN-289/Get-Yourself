#!/usr/bin/env node
/**
 * verify-pipeline.mjs — Health check for get-yourself-cli pipeline integrity
 *
 * Checks:
 ...
 *
 * Run: node get-yourself-cli/verify-pipeline.mjs
 */
...
// GET_YOURSELF_TRACKER overrides the path (used by tests and non-standard layouts).
const APPS_FILE = resolveTrackerPath(GET_YOURSELF);

const ADDITIONS_DIR = join(GET_YOURSELF, 'batch/tracker-additions');
// GET_YOURSELF_REPORTS overrides the reports dir (used by tests, mirrors GET_YOURSELF_TRACKER).
const REPORTS_DIR = process.env.GET_YOURSELF_REPORTS
  ? resolve(GET_YOURSELF, process.env.GET_YOURSELF_REPORTS)
  : join(GET_YOURSELF, 'reports');
...
for (const e of entries) {
  ...
  if (!match) continue;
  const link = match[1];
  if (!existsSync(join(TRACKER_DIR, link)) && !existsSync(join(GET_YOURSELF, link))) {
...
//
// Path resolution deliberately matches the two consumers (GET_YOURSELF/data/...)
// rather than APPS_FILE's directory: the check exists to predict what they will
// do, so it has to read the same file they read.
const FOLLOWUPS_FILE = join(GET_YOURSELF, 'data', 'follow-ups.md');
