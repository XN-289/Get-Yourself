...
| `data/scam-check-backups/*` | 显式替换防骗核查包或报告前的备份 |
| `reports/scam-check/*.md` | 由防骗核查包派生的本地报告 |
...
| `data/sync-queue.json` | 用户确认后的本地显式同步摘要队列（只表示待发，不代表已上传） |
| `data/sync-queue-backups/*` | 队列每次显式替换前的有界备份 |
| `data/status-log.tsv` | 状态流转日志（追加式） |
| `data/skill-runs/*.json` | 用户确认后的本地 Skill Runtime 审批记录 |
| `data/skill-run-backups/*` | 显式替换 Skill Runtime 审批记录前的备份 |
| `reports/*` | 评估报告 |
| `output/*` | 生成的 PDF |
