...
/**
 * Returns the resolved get-yourself-cli data directory root.
 * Priority: process.env.GET_YOURSELF_ROOT or process.env.GET_YOURSELF_DATA_DIR >
 * .get-yourself-cli-data marker file > codebase root (__dirname).
 * 
 * @returns {string} Absolute path to the data root
 */
export function getCareerOpsRoot() {
  const env = process.env.GET_YOURSELF_ROOT?.trim() || process.env.GET_YOURSELF_DATA_DIR?.trim();
  if (env) {
    return resolve(__dirname, env);
  }
  const markerFile = join(__dirname, '.get-yourself-cli-data');
 ...
 * Priority: process.env.GET_YOURSELF_TRACKER > root/data/applications.md > root/applications.md.
 *
 * @param {string} rootDir The get-yourself-cli data root directory
 * @returns {string} Canonical absolute path to the tracker file
 */
export function resolveTrackerPath(rootDir) {
  const env = process.env.GET_YOURSELF_TRACKER?.trim();
  const raw = env
    ? env
 ...
 * Priority: process.env.GET_YOURSELF_TRACKER > root/data/applications.md.
 * Does not check for file existence, providing a deterministic write target.
 * 
 * @param {string} root The get-yourself-cli data root directory
