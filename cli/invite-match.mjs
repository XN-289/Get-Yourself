#!/usr/bin/env node
/**
 * invite-match.mjs — Interview-Invite / Rejection → Tracker Matcher for get-yourself-cli
 *
 ...
 *
 * Issue #1495, #2098 — github.com/santifer/get-yourself-cli
 */
 ...
 * @param {{appsFile?: string}} [options] - appsFile overrides GET_YOURSELF_TRACKER for the child process (tests only).
...
export function applyRejectionStatus(appNumber, options = {}) {
  const scriptPath = join(GET_YOURSELF, 'set-status.mjs');
  const env = options.appsFile ? { ...process.env, GET_YOURSELF_TRACKER: options.appsFile } : process.env;
  try {
...
function printSummary(result) {
  console.log(`\n${'='.repeat(70)}`);
  console.log('  Interview Invite / Rejection Matcher — get-yourself-cli');
