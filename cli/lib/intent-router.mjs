...
const ROUTES = {
  ...
  resumeRender: {
    intent: 'render_resume',
    displayName: '渲染简历模板',
    moduleDestination: 'resume-management',
    modeFile: 'resume-render.mjs',
    suggestedAction: '先把简历事实整理成结构化渲染包并确认模板；再执行 check 和 import dry-run，用户确认后显式 --apply。',
    needsConfirmation: true,
    fallbackPrompt: '请提供要渲染的简历内容或已确认的 render.json，并说明想用哪个模板。',
    securityNotes: ['渲染包是唯一事实来源，模板示例不得进入简历；HTML 只保存在本地，不上传、不自动打开浏览器。'],
  },
  capability: {
