...
2026-09-04 PRD r10 补充：`docs/PRODUCT_DESIGN_V0.1.md` 新增模块大白话进度表，按 Agent、能力资产、简历、面试、本地 Skill Runtime、Stage 6 本地同步准备、平台协同与后端拆开“现在能干什么 / 机器预检 / 用户还验什么”。PRD 同时记录 A1-A7 均有机器预检但用户结论仍为待验收、后端 Maven 回归仍被本机缺 Maven 阻断。本轮只做进度口径校准，不改变产品目标、模块边界、6c 双门槛或任何验收结论。
