...
| Stage 6 | 进行中（仅合同基线） | 同步单元合同 v0.1 已定义；求职摘要上传、网页进度投影、Trace 汇总与重试闭环尚未实现 |
...
| 合同基线已落盘、实现未开始 | 首批同步单元限定为公司机会进度摘要与 Trace 决策摘要 | 已定义自然身份、内容指纹、basis 冲突、append-only Trace、多设备合并、显式队列、重试、删除墓碑与隐私红线；尚无 API、数据库、CLI 队列或前端同步实现，不能描述为自动同步已开始 |
