...
| 项目 | 内容 |
|---|---|
| 文档状态 | Agent-first 实现校准修订版，含 2026-09-03 实现进度快照与 Stage 6 实施顺序；待产品与技术评审与用户统一验收 |
| 产品阶段 | v0.1 最小闭环设计 |
...
| Stage 6 | 进行中（仅合同基线） | 同步单元合同 v0.1 已定义；下一步先做本地确定性构建器 / 校验器，求职摘要上传、网页进度投影、Trace 汇总与重试闭环尚未实现 |
...
| 合同基线已落盘、实现未开始 | 首批同步单元限定为公司机会进度摘要与 Trace 决策摘要 | 已定义自然身份、内容指纹、basis 冲突、append-only Trace、多设备合并、显式队列、重试、删除墓碑与隐私红线；下一步是纯本地 builder / validator，尚无 API、数据库、CLI 队列或前端同步实现，不能描述为自动同步已开始 |
...
Stage 6 当前只有合同基线：`docs/SYNC_UNIT_CONTRACT.md` 把首批同步单元收窄为公司机会进度摘要与 Trace 决策摘要。前者按公司、岗位、地点、招聘批次和 schema 版本构成自然身份，是携带 basis 指纹的可变更快照；后者按来源设备与 Trace ID append-only。两种单元都必须经用户显式确认才入队；简历全文、JD 原文、报告全文、STAR 原稿、私人备注、联系人和原始材料永不进入同步单元。

Stage 6 的实施顺序固定为：先落本地确定性构建器与校验器，再落用户确认后的本地 pending 队列，然后才开放设备授权 API、网页投影和 Trace 汇总。前置本地步骤不联网、不写用户层、不创建同步入口；只有八类合同门槛测试全部通过后，才进入接口与持久化设计。
...
| 里程碑 | 范围 | 当前边界 |
|---|---|---|
| 6a 本地确定性构建 / 校验 | 构建两类摘要单元，计算自然身份、内容指纹与幂等键；分类 first / idempotent / accepted / conflict；校验禁传字段 | 下一步实现。纯函数与测试先行，不联网、不写用户层、不做 API |
...
当前实现边界：Stage 6 尚未开始写代码。6a 完成前不得声称同步单元可构建、可入队或可上传；6b 完成前不得创建用户可见 pending 队列；6c 完成前不得修改设备绑定语义或数据库；6d 完成前网页不得展示会误导用户以为已同步的进度投影。
