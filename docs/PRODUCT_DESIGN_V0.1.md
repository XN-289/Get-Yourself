...
| 项目 | 内容 |
|---|---|
| 文档状态 | Agent-first 实现校准修订版 r6，含简历对象化管理与选区兜底 skill、2026-09-03 实现进度、Stage 6a 本地确定性构建 / 校验与 Stage 6b 显式本地队列状态、下一步统一验收与 6c 门槛；待产品与技术评审与用户统一验收 |
| 产品阶段 | v0.1 最小闭环设计 |
...
| 进度分类 | 当前范围 | 关键边界 |
|---|---|---|
| 已实现，待用户统一验收 | Agent-first 前端 Demo、四个独立模块、`gy` 本地入口、能力证据包显式导出 / 导入、设备绑定、简历对象化管理、简历素材与定稿、简历版本库文件桥、选区兜底 skill、显式事实链身份绑定与只读审计、面试准备与复盘、结构化简历渲染、本地能力反哺台账 | Stage 1 到 Stage 4 的仓库实现已完成，但用户尚未明确验收 |
...
| 顺序 | 事项 | 边界 |
|---|---|---|
| 1 | 准备用户统一验收包 | 覆盖 Agent、能力资产、简历管理、面试管理、本地 Runtime、6a 与 6b；仓库测试通过不等于用户验收 |
...
这里的“通过测试”只代表仓库验证完成，不代表用户验收完成。用户统一验收可以开始，但不因 Stage 推进而自动通过；若验收发现体验阻断，修复优先级高于继续扩展云端能力。
...
| 6b 显式本地队列 | 只保存用户确认过的摘要；提供用户可见的队列状态、取消、认证阻断、重绑阻断、冲突处理和用户触发重试 | 已实现并待用户统一验收。9 个专项测试与 CLI 全量测试通过；仍不联网、不做自动重试或自动上传 |
...
当前实现边界：Stage 6a 已完成本地纯函数 builder / validator 与测试，Stage 6b 已完成显式队列持久化、CLI 操作、审计、备份和 `gy --status` 摘要。队列中的 pending 只表示“本地待发”，不表示已上传或已同步；6c 完成前不得修改设备绑定语义、API 或数据库；6d 完成前网页不得展示会误导用户以为已同步的进度投影。
...
| Stage 5 | 公司机会流程轨、节点抽屉、JD 分析、防骗核查、公司机会与 tracker 桥接、节点 mutation、产物挂载、前端文件桥、页面内确认流、Runtime 全量契约桥 | 云端同步、自动状态推进或未注册工具执行 |
...
3. 简历管理：成品导入、简历对象列表、唯一草稿派生与保存、选区兜底 skill、定稿、当前投递版、导出标记和事实链状态。
...
| 验收与实施门槛 | 6a 测试通过只代表仓库验证；6b 可与用户统一验收并行；进入 6c 前必须完成 6b 仓库测试和用户本地侧验收 | 防止云端能力扩展压过体验阻断和数据边界问题 |
