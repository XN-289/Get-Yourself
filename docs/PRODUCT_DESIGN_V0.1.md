...
| 项目 | 内容 |
|---|---|
| 文档状态 | Agent-first 验收校准修订版 r7，含 2026-09-04 实现 / 验收边界、简历对象化管理与选区兜底 skill 的人工验收口径、Stage 6a 与 6b 的仓库验证边界、统一验收顺序与 6c 门槛；待产品与技术评审与用户统一验收 |
| 产品阶段 | v0.1 最小闭环设计 |
...
Stage 6 的实施顺序固定为：本地确定性构建器与校验器、用户确认后的显式本地队列均已落地并通过仓库测试，下一步必须先完成用户统一验收，然后才开放设备授权 API、网页投影和 Trace 汇总。6a 不联网、不写用户层；6b 只写队列文件与队列备份，仍不联网、不上传、不自动重试。
...
| 3 | 6b 完成用户本地侧验收后，才进入 6c 授权 API 设计 | 云端接口、数据库和网页投影不得提前实现 |
| 4 | 6c 后才进入 6d 网页摘要与 Trace 汇总 | 网页只读展示，不反向编辑本地权威对象 |
...
| 6c 授权 API 与云端投影 | 设备授权下接收同步单元，落幂等记录、冲突队列与删除墓碑 | 6b 完成仓库测试并通过用户本地侧验收后，才设计接口、数据库和管理端；不复制本地对象 |
...
当前实现边界：Stage 6a 已完成本地纯函数 builder / validator 与测试，Stage 6b 已完成显式队列持久化、CLI 操作、审计、备份和 `gy --status` 摘要。两者均已通过仓库测试，但均未获得用户验收。队列中的 pending 只表示“本地待发”，不表示已上传或已同步；在用户完成 6a / 6b 本地侧统一验收前，不得启动 6c 授权 API、数据库、云端投影或上传重试闭环；6c 完成前不得修改设备绑定语义；6d 完成前网页不得展示会误导用户以为已同步的进度投影。
...
| 验收与实施门槛 | 仓库实现、测试和构建通过均不代表用户验收；进入 6c 前必须完成 6b 仓库测试和用户本地侧验收 | 防止云端能力扩展压过体验阻断和数据边界问题 |
