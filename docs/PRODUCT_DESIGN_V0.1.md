...
| 项目 | 内容 |
|---|---|
| 文档状态 | 后端 P0 加固修订版 r12，含实现 / 机器预检 / 用户验收三层边界、简历对象化管理与选区兜底 skill 的人工验收口径、Stage 6a 与 6b 的仓库验证边界、后端路由矩阵与 Trace 摘要化、后端 P0 验证证据、6c 双门槛与独立验收标准；待产品与技术评审与用户统一验收 |
| 产品阶段 | v0.1 最小闭环设计 |
...
| 后端架构 | 网页后端保持模块化单体，云侧与本地侧权威边界固定 | 不把本地工作台改造成第二个云端业务库，也不因架构掌控启动 6c |
...
| 阶段 | 当前状态 | 说明 |
|---|---|---|
| Stage 0 | 文档已校准，P0 加固当前范围已验证，待评审 | 产品、契约与后端架构控制基线已形成文档；路由矩阵、Trace 摘要化、49 项后端回归与 MariaDB V22→V23 迁移已完成当前工程验证。BA-01 冻结域公共入口与集中式策略仍待处置，MySQL 8.4 与完整启动链路未验证，且仍需用户门槛确认 |
...
| Stage 4 | 已实现，待用户统一验收 | 简历对象化管理、简历素材包、简历定稿审批、面试准备清单、面试复盘、结构化简历渲染、简历版本库文件桥、选区兜底 skill、显式事实链身份绑定与只读审计、本地能力反哺台账已落地；平台同步不属于本阶段 |
| Stage 5 | 进行中（本地主体已实现，待用户统一验收） | 公司机会流程轨、本地 JD 分析合同 / 报告、公司机会对象、tracker 持久化、节点 mutation 合同、真实产物挂载、防骗核查契约、前端显式文件桥、页面内 skill 确认流、Skill Runtime v0.1 审批账本与 v0.2 全量 11 条契约执行桥已落地；云端同步未完成 |
| Stage 6 | 进行中（6a、6b 已实现并通过机器预检，待用户验收；6c、6d 未开始） | 同步单元合同 v0.1、本地确定性构建器 / 校验器与显式本地队列已落地并通过测试；授权 API、数据库、云端投影、网页 Trace 汇总与上传重试闭环尚未实现 |

#### 0.0.1 2026-09-04 当前实现与验收快照

当前仓库进度分为四类，均不等同于用户验收或产品可上线：

| 进度分类 | 当前范围 | 关键边界 |
|---|---|---|
| 已实现，待用户统一验收 | Agent-first 前端 Demo、四个独立模块、`gy` 本地入口、能力证据包显式导出 / 导入、设备绑定、简历对象化管理、简历素材与定稿、简历版本库文件桥、选区兜底 skill、显式事实链身份绑定与只读审计、面试准备与复盘、结构化简历渲染、本地能力反哺台账 | Stage 1 到 Stage 4 的仓库实现已完成，并具备仓库验证证据；这不代表用户验收、体验通过或产品可上线 |
...
| 本地确定性队列已实现、云端链路未开始 | 首批同步单元限定为公司机会进度摘要与 Trace 决策摘要，Stage 6a builder / validator 与 6b 显式队列已落地 | 已实现自然身份、内容指纹、幂等键、冲突分类、Trace append-only 冲突识别、本地队列持久化、状态过滤、取消、用户触发重试、重绑确认、冲突证据与删除墓碑防护；尚无上传 API、数据库或前端同步实现，不能描述为自动同步或上传能力已开始 |
| 后端 P0 加固已验证当前范围、6c 仍未解锁 | 85 条路由 / 23 个请求 Controller 的鉴权与所有权矩阵已建立；Agent Trace 新写入改为仅存脱敏摘要，历史数据由 V23 重写；设备令牌边界经代码矩阵复核仍仅限状态与断开；后端回归 49/49 通过，MariaDB 11.4 V22→V23 迁移和敏感残留断言通过 | BA-01 的冻结域公共写入口和集中式运行时策略仍未处置；BA-03 只完成边界澄清；MySQL 8.4 和后端完整启动链路未验证；这些缺口未收口前不得把这批登记为 6c 通过 |

#### 0.0.2 2026-09-04 模块大白话进度

下表回答“每个模块现在到底做到哪里”。机器预检只说明当前仓库行为可以被自动化检查复现，不等于用户验收，也不等于产品可上线。

| 模块 | 现在能干什么 | 机器预检结果 | 用户还必须验什么 |
|---|---|---|---|
| Agent 工作台 | 作为学生侧主入口理解固定意图，先出写入计划，确认后落模块对象，取消则不写入；能力资产、简历、面试仍是独立落点 | A1 通过：确认生成草稿、取消不写入、JD 注入保持只读、独立模块可打开、窄屏无溢出 | 对话手感、计划文案是否看得懂、目标模块落点是否符合心智 |
| 能力资产（暂名） | 网页按年份和方向导出证据包；本地 CLI 校验、dry-run、显式 apply 和幂等导入；面试复盘与 JD 差距先作为候选证据，不伪装成已评分 | A2 通过：真实临时后端导出、CLI 校验与导入、重复 apply 幂等、网页评分无反向改写 | 用自己的真实账号走一遍导出 / 导入，确认候选证据和评分边界能理解 |
| 简历管理 | 一份简历是唯一主对象；支持成品导入、唯一草稿、手工与 Agent 修改汇流、证据门槛兜底 skill、定稿、历史切换、版本库导出和 CLI 安装 | A3 通过：普通选区与右键拖拽在未滚动、滚动后命中同一可见文本；build 通过 | 用户用真实鼠标验手感、编辑效率、选区上下文、Agent 建议是否敢用 |
| 面试管理 | 以公司机会为主对象，横向流程轨展开 JD 分析、简历、投递、笔试、面试、Offer 和沉淀；节点可加、可拖、可左移，状态必须人工确认，产物可挂载 | A4 通过：按钮先开抽屉、确认才改状态，节点变化不跨公司，Offer 有鼓励反馈，窄屏仅流程轨内部滚动 | 用户用自己的投递流程验信息密度、拖拽手感、节点顺序和鼓励反馈 |
| 本地 Skill Runtime | 7 个封闭 skill 和 11 条契约执行桥可 list、check、dry-run、apply；执行前后有指纹、备份、幂等与冲突拒绝，未注册目标被拒绝 | A5 通过：示例 dry-run 不落盘，显式 apply 写声明目标，重复 apply 不变，同 runId 冲突被拒绝 | 用户确认审批文案、目标边界、失败恢复说明是否符合信任预期 |
| Stage 6 本地同步准备 | 6a 能构建并校验公司机会进度摘要与 Trace 决策摘要；6b 能把这些摘要放入显式本地队列，支持幂等、取消、重试、重绑确认和冲突证据 | A6 / A7 通过：6a 专项 14/14，6b 专项 9/9；list 与状态明确无联网、无自动上传、无自动重试、不改业务对象 | 用户确认冲突口径与“pending 只是本地待发”的心智；6c 仍禁止启动 |
| 平台协同与后端 | 已有模块化单体架构控制、云本地权威边界和风险清单；85 条路由的鉴权 / 所有权 / 写入矩阵可审查；Trace 新写入只保存脱敏摘要，历史 artifact 由 V23 重写 | 后端回归 49/49 通过；MariaDB 11.4 完成 V22→V23 迁移、legacy 摘要化、哈希重算和敏感残留 0 断言；迁移后的应用启动因 Hibernate 方言识别失败，且 MySQL 8.4 未验证 | 用户确认冻结域公共写入口如何处置；6c 设计前仍必须通过 6b 用户验收 + BA-01 至 BA-03 处置，并补齐目标数据库 / 启动链路证据 |

简历管理当前已形成“一份简历是一个对象”的前端 Demo：左侧平铺展示简历对象，目标岗位、模板、导入来源和历史都是该对象的元数据或次级面板；每份对象支持当前投递版、唯一待处理草稿、草稿派生、定稿、导出标记、历史投递版切换和本机成品导入。手工修改与 Agent 修改汇入同一份草稿，定稿、导出与当前投递版切换仍必须显式确认。简历对象与版本目录可通过 `get-yourself.resume-library v1` 显式导出，经 CLI 校验、dry-run 与 `--apply` 安装到本地；该版本库不替代 `cv.md` 定稿权威，也不自动生成渲染 HTML 或同步云端。对抗检验后，契约身份规则为外部显式 ID 优先锁定、本地默认 ID 避让，时间统一为毫秒精度 UTC，`generatedAt` 与 `traceId` 不参与语义哈希，因此仅时间或 Trace 变化的重导出应保持幂等。渲染包与版本库支持成组的可选定稿 / 渲染 / 导入来源指纹，且这些来源指纹参与语义哈希：旧文件继续输出 `binding-gap`，显式匹配当前定稿可被事实链证明，显式过期绑定输出漂移；前端导入保留真实来源指纹但不伪造渲染哈希。当前投递版按自身定稿绑定、渲染绑定和 `cv.md` 全文比较给出对象状态，集合汇总只做聚合，不把已漂移的子对象降级成 `ready`。编辑器内可选中片段并触发兜底 skill；该 skill 只在能力资产存在匹配证据时生成保守替换稿，替换先落在编辑缓冲区，保存前不写入任何简历版本。以上是仓库实现与自动化验证结论；真实鼠标右键拖拽、编辑器滚动后的选区映射、上下文是否正确使用当前简历与能力资产、以及建议生成 / 替换 Trace 是否符合体验预期，仍必须由用户统一验收确认。

Stage 6 当前已从合同基线推进到 6b：`docs/SYNC_UNIT_CONTRACT.md` 把首批同步单元收窄为公司机会进度摘要与 Trace 决策摘要。前者按公司、岗位、地点、招聘批次和 schema 版本构成自然身份，是携带 basis 指纹的可变更快照；后者按来源设备与 Trace ID append-only。`cli/sync-unit.mjs` 提供本地确定性构建、canonical 哈希、严格校验、冲突分类、队列动作纯规则和墓碑防护；`cli/sync-queue.mjs` 在此之上提供本地持久化队列、dry-run / apply、状态列表、取消、用户触发重试、认证与重绑阻断、冲突证据、删除墓碑和 `gy --status` 摘要。队列仍不联网、不自动上传、不后台重试、不改本地业务对象；简历全文、JD 原文、报告全文、STAR 原稿、私人备注、联系人和原始材料永不进入同步单元。

Stage 6 的实施顺序固定为：本地确定性构建器与校验器、用户确认后的显式本地队列均已落地并通过仓库测试，下一步必须先完成用户统一验收和后端架构 P0 门槛复核，然后才开放设备授权 API、网页投影和 Trace 汇总。6a 不联网、不写用户层；6b 只写队列文件与队列备份，仍不联网、不上传、不自动重试。
...
| 顺序 | 事项 | 边界 |
|---|---|---|
| 1 | 准备并执行用户统一验收包 | 覆盖 Agent、能力资产、简历管理、面试管理、本地 Runtime、6a 与 6b；仓库测试和构建通过不等于用户验收，已实现范围仍不得记录为“已验收” |
| 2 | 修复统一验收中的体验阻断 | 验收结论按通过 / 需修改 / 阻断记录，不能被云端进度覆盖 |
| 3 | 完成后端架构 P0 门槛复核 | 当前范围已完成工程验证：路由矩阵、后端回归 49 pass、MariaDB 11.4.13 V22→V23 迁移与敏感残留检查通过；MySQL 8.4、完整启动链、BA-01 冻结域公共入口集中处置与用户验收仍开放，设备令牌仍只授权状态与断开，6c 逐单元授权边界不变 |
| 4 | 6b 用户本地侧验收与后端架构 P0 门槛都通过后，才进入 6c 授权 API 设计 | 云端接口、数据库和网页投影不得提前实现 |
| 5 | 6c 后才进入 6d 网页摘要与 Trace 汇总 | 网页只读展示，不反向编辑本地权威对象 |

这里的“通过测试”“构建通过”或“仓库实现完成”只代表仓库验证完成，不代表用户验收完成。用户统一验收可以开始，但不因 Stage 推进而自动通过；若验收发现体验阻断，修复优先级高于继续扩展云端能力。
...
后端架构以 `docs/BACKEND_ARCHITECTURE_CONTROL_V0.1.md` 为当前控制基线。产品侧只从该基线继承约束，不把技术盘点误写成新能力。

| 控制项 | v0.1 结论 | 产品含义 |
|---|---|---|
| 系统形态 | 网页后端保持 Spring Boot 模块化单体 | 本阶段不拆微服务，不在本地或前端另建平行后端 |
| 云侧权威 | 账号、成长记录、能力证据与评分、平台记忆、Agent Trace、设备授权 | 网页继续承担大脑和证据中枢 |
| 本地权威 | 简历对象与版本、素材、STAR、机会流程、Skill Runtime 产物、未确认队列 | 浏览器和云端不得直接改写本地文件 |
| 变更入口 | 新能力先声明模块、路由、身份、所有权、写入数据、冻结域影响和 Trace 行为 | 防止活动、组织、日程等旧路线回流 |
| 异步边界 | 业务事务产生的异步动作走事务 outbox | 避免评分、记忆或同步派生动作丢失后无法解释 |
| AI 边界 | LLM 只提出材料或解释，确定性代码负责校验、评分、状态和持久化 | Agent 不能自动改写用户确认的求职结果 |
| 路由矩阵 | `docs/BACKEND_ROUTE_AUTH_MATRIX_V0.1.md` 与代码同批更新，逐条记录身份、所有权和写入 | 新路由没有矩阵行不得合入；矩阵不是已集中鉴权的证据 |
| Trace 摘要 | 平台 Trace artifact 只保存受控类型和脱敏决策摘要，历史数据经 V23 摘要化 | 调用方仍不得把简历全文、STAR 原稿、凭证或原始个人文件塞进摘要 |

进入 Stage 6c 前，除原有 6b 用户本地侧验收外，还必须完成后端架构 P0 门槛复核：

1. BA-01：85 条路由的身份、所有权和写入规则有代码对照矩阵；同时明确处置冻结域仍暴露的公共变更 / 重建索引入口，不能把“登记风险”当成“生产可接受”。
2. BA-02：Trace 新写入只保存脱敏摘要，历史 artifact 经 V23 重写；当前工程范围已通过 49/49 后端回归和 MariaDB 11.4 V22→V23 迁移验证，但 MySQL 8.4 与迁移后的完整启动链路仍未验证。
3. BA-03：设备 token 只能用于设备状态与断开；未来接收同步摘要必须逐单元显式授权、幂等、可冲突、可 Trace，不能成为第二个网页登录态。

这三项可以被工程修复，也可以由用户基于风险明确接受；不能因为架构文档已经登记而默认通过。
...
| 平台 Agent Trace 索引与摘要 | 网页 | 平台侧调用索引、状态和用户确认后的决策摘要 |
| 本地 Trace 完整上下文 | 本地 | 原始上下文、完整产物索引和本地执行细节不上传 |
| 简历定稿 | 本地 | 用户可本地维护和版本化 |
| 原始个人材料 | 本地 | 不自动上传 |
...
| 已有旧简历 | 本机有 `.md`、`.txt`、`.html` 或合规渲染 JSON，但没有平台证据 | 一份可管理的简历对象 v1 和待补证据清单 | 用户导入成品简历，系统建立简历对象与当前投递版，再列出无法证明的表达 | 不推断新事实，不自动改写原文 |
...
简历管理是“成品简历对象库”，唯一可见主对象是一份已经能交付给市场的完整简历，而不是候选素材、事实确认任务、模板说明页或岗位分组。素材生成、事实确认和渲染审批仍属于 Agent 生成链路；一旦形成完整简历，才进入本模块。

管理结构分为两层：

1. **简历对象**：一份面向某个求职目标的完整简历，例如“Java 后端主简历”或“前端实习一页版”。用户在列表中直接看到、选择、导入、派生和定稿的都是这份对象。
2. **版本**：同一份对象下的 v1、v2、v3 等历史版本，每个版本记录状态、模板、来源、更新时间、版本说明和全文。

目标岗位、模板、导入来源、文件索引、历史版本和当前投递版是简历对象的元数据或次级面板，不再作为主导航层级。岗位分组不能挡在用户和简历对象之间；同一岗位可以有多份不同策略的简历，一份简历也可以在用户确认后调整目标岗位。

每份简历对象有且只有一个“当前投递版”。当前投递版只能从已定稿或已导出版本中选择；草稿不能投递，也不能因为新简历只有草稿就被伪装成可投递版本。用户可以在历史面板中预览版本，并将某个非草稿历史版本显式设为当前投递版。

| 区域 | 内容 |
|---|---|
| 简历对象列表 | 平铺展示每份简历，直接显示标题、目标岗位、当前投递版状态和待处理草稿状态 |
| 对象详情 | 简历名称、目标岗位、当前投递版、待处理修改、版本数量、文件名、模板与全文预览 |
| 版本动作 | 草稿可编辑和定稿；定稿可标记导出；非草稿历史版可设为当前投递版 |
| 简历编辑 | 定稿和已导出版只读；用户修改名称、目标岗位、模板、版本说明和完整正文时必须派生或打开这份对象的唯一草稿 |
| 导入入口 | 支持导入本机 `.json`、`.md`、`.txt`、`.html` 成品文件；JSON 仅接受用户确认过的 `get-yourself.resume-render v1` 渲染包；导入建立一份新简历对象，首版为已定稿并设为当前投递版 |
| 生成入口 | Agent 生成或更新一份简历对象中的唯一草稿版本，而不是只产出候选 bullet，也不能覆盖定稿或已导出版 |
...
- 草稿必须经用户确认定稿，定稿后自动成为这份简历的当前投递版。
- 定稿与已导出版不可直接编辑；继续修改必须派生新草稿。
- Agent 只能创建或更新草稿，不能覆盖用户已定稿或已导出的版本。
- 手工修改与 Agent 修改共享同一份草稿；两者不能各自生成互相冲突的待处理版本。

导入和编辑都在本地读取或维护简历全文，不上传、不自动同步、不触发云端改写；模板只影响版式，不新增或推断事实。用户明确选择上传或同步某一版时，必须先看到该版全文和目标范围。

#### 9.5.1 选区兜底 skill

简历编辑器必须支持片段级修改：用户可以在正文中选中一个表达片段后右键触发 Agent 改写，右键拖拽也应能形成选区。选区上下文固定为“当前简历全文 + 全部能力资产”，但上下文只提供证据，不提供编造许可。

兜底 skill 是证据门槛型，不执行自由生成：

1. 未选中文本、选区超过 600 字、选区包含标题行或选区已随全文修改过期时阻断。
2. 能力资产为空，或没有与选区、简历内容和修改意图匹配的证据时阻断。
3. 有匹配证据时，只能生成保守替换稿：一次处理一条经历或一个表达片段，保留原 bullet 语法，标出支撑能力；无可核验数字时明确输出“量化结果待补充”。
4. skill 不得新增事实、指标、经历、公司、职位、时间或工具；证据缺口必须作为缺口展示。
5. 生成建议与执行替换分别记录 Trace；替换只修改编辑缓冲区，用户仍需显式保存草稿，保存前当前投递版和定稿版本不变。

当前实现与验收边界：选区兜底 skill 的阻断、保守建议、编辑缓冲区替换、草稿保存和当前投递版保护已完成仓库验证。真实右键拖拽无法被当前自动化接口可靠替代，必须在用户统一验收时用真实鼠标验证；验收至少覆盖未滚动编辑器、滚动后编辑器、当前简历全文上下文、能力资产证据上下文、建议生成 Trace、替换 Trace、取消路径和显式保存路径。
...
| 简历版本库 | 简历对象、版本、状态和当前投递版的工作台目录 | 不是简历事实权威 |
...
| 版本库导入外部成品 | 建立新简历对象，不反向改写素材或 `cv.md` |
...
16. 后端架构控制：模块化单体、云本地权威、鉴权矩阵、Trace 脱敏语义和 Stage 6c 前置门槛。
...
- 不复用活动关系作为求职关系。
- 产品文案不再以活动发现为主叙事。
- 后端新增或改造能力先对照模块 / 路由归属、数据权威和架构风险清单。
...
风险：如果鉴权、Trace 脱敏和设备授权边界等到云端同步实现后才补，6c 会把旧风险放大成跨端数据事故。

对策：

- 把后端架构 P0 门槛放在 6c 设计之前。
- 架构文档只登记风险，不等于风险已处置。
- 用户明确接受风险时必须记录范围、期限和补偿措施。
- 修复后用后端回归验证，不用 PRD 或评审记录替代测试。
...
- 团队能一致回答数据放在哪里、谁能修改、如何同步、后端风险何时必须处置。
...
前端简历管理已按“一份简历对象 / 版本”改造：页面提供平铺对象列表、当前投递版、待处理草稿、本地导入、草稿派生、草稿保存、定稿、导出标记、历史抽屉和历史投递版切换；Agent 生成只写入当前对象的唯一草稿。编辑器支持选中文本触发证据门槛型兜底 skill，右键拖拽形成的选区同样进入该规则，替换只落在编辑缓冲区。2026-09-03 晚间的仓库验证覆盖无证据阻断、有证据保守建议、证据缺口展示、编辑缓冲区替换、草稿保存、当前投递版保护和保存 Trace；其中右键拖拽只完成实现复查，仍待真实鼠标验收，390px 回归也待统一验收时复测。简历对象与版本目录已通过 `get-yourself.resume-library v1` 显式文件桥持久化：浏览器只导出或读取契约 JSON，写入本地必须经 CLI check、dry-run、`--apply` 与替换时的 `--replace`。该版本库只是工作台目录权威，不替代 `cv.md`、素材或定稿计划；显式来源指纹只用于事实链审计，导入版本没有反写权限。
...
| 里程碑 | 范围 | 当前边界 |
|---|---|---|
| 6a 本地确定性构建 / 校验 | 构建两类摘要单元，计算自然身份、内容指纹与幂等键；分类 first / idempotent / accepted / conflict；校验禁传字段 | 已完成仓库实现与测试，待用户统一验收。`cli/sync-unit.mjs` 与 14 个专项测试完成，全量 CLI 测试 120 通过 / 0 失败；仍不联网、不写用户层、不做 API、不提供用户可见队列 |
| 6b 显式本地队列 | 只保存用户确认过的摘要；提供用户可见的队列状态、取消、认证阻断、重绑阻断、冲突处理和用户触发重试 | 已完成仓库实现与测试，待用户本地侧验收。9 个专项测试与 CLI 全量测试通过；仍不联网、不做自动重试或自动上传 |
| 6c 授权 API 与云端投影 | 设备授权下接收同步单元，落幂等记录、冲突队列与删除墓碑 | 6b 通过用户本地侧验收，且后端架构 P0 门槛被修复或由用户明确接受后，才设计接口、数据库和管理端；不复制本地对象 |
...
当前实现边界：Stage 6a 已完成本地纯函数 builder / validator 与测试，Stage 6b 已完成显式队列持久化、CLI 操作、审计、备份和 `gy --status` 摘要。两者均已通过仓库测试，但均未获得用户验收。队列中的 pending 只表示“本地待发”，不表示已上传或已同步；在用户完成 6a / 6b 本地侧统一验收且后端架构 P0 门槛被修复或由用户明确接受前，不得启动 6c 授权 API、数据库、云端投影或上传重试闭环；6c 完成前不得修改设备绑定语义；6d 完成前网页不得展示会误导用户以为已同步的进度投影。

Stage 6b 的产品交付不是“上传功能”，而是本地可信待发队列：

| 能力 | 用户可见结果 | 禁止行为 |
|---|---|---|
| 摘要确认 | 用户先看到单元类型、对象、语义字段、指纹和不上传范围，确认后才入队 | 未确认自动入队 |
| 队列状态 | 每条待发摘要可查看等待、认证阻断、重绑阻断、冲突和待处理状态；取消动作有明确反馈和可追溯记录 | 后台静默重试 |
| 冲突承接 | 冲突条目展示本地与云端基准差异，用户刷新并选择新基准后才能重新确认 | 原样重试冲突 |
| 取消 | 用户取消只删除队列条目并留下可追溯状态 | 修改本机会、tracker、节点、产物或简历 |
| 隐私 | 队列文件只保存摘要和指纹 | 保存简历全文、JD 原文、报告全文、私人备注或联系人 |
...
以下清单是用户验收标准，不因仓库实现完成或测试通过而自动勾选。“仓库验证”只证明当前代码满足确定性测试；“用户验收”必须由用户在真实界面和本地命令中确认体验、心智和结果正确。

截至 2026-09-04 15:08，A0 已完成环境基线记录，A1 到 A7 均已有一轮机器预检；但这些证据仍不改变任何用户验收结论。A0 当时因 PATH 缺 Maven 记录为阻断。后续后端 P0 加固批次使用仓库外隔离 JDK / Maven 完成 49/49 回归，并用隔离 MariaDB 完成迁移验证；这证明本批次工程行为，但不改变标准环境 A0 结论，也不能替代用户验收。

| 范围 | 当前可验收内容 | 不能据此验收 |
|---|---|---|
| Stage 1 到 Stage 4 | Agent-first 四模块、`gy` 入口、证据包导出 / 导入、设备绑定、简历素材与定稿、版本库、事实链、面试准备与复盘、能力反哺 | 自动同步、平台评分回写或用户体验已通过 |
| Stage 5 | 公司机会流程轨、节点抽屉、JD 分析、防骗核查、公司机会与 tracker 桥接、节点 mutation、产物挂载、前端文件桥、页面内确认流、Runtime 全量契约桥的已实现本地部分 | 云端同步、自动状态推进、未注册工具执行，或把仓库测试通过等同于用户验收 |
| Stage 6a | 两类同步单元 builder / validator 的测试证据、无写入和无联网边界 | 队列持久化、上传、网页投影、Trace 汇总或用户可见重试 |
| Stage 6b | 本地队列 dry-run / apply、幂等入队、状态过滤、取消、认证与网络重试、重绑确认、冲突证据、删除墓碑和无联网边界 | 上传、云端确认、网页投影、自动重试或用户验收通过 |
| 后端架构基线 | 模块化单体、云本地权威、模块 / 路由归属、风险清单和变更规则的可审查文档 | 后端重构完成、安全风险已修复、Stage 6c 已开始或用户验收通过 |

当前统一验收不要求一次测完所有命令，但必须按模块记录结论：通过、需修改或阻断。任何“需修改”和“阻断”在进入 6c 前修复或明确降级，不能用后续云端进度覆盖。
...
验收按“先看主入口，再验模块对象，最后验本地合同”的顺序执行，避免把界面演示和本地执行混成一团：

验收标准：`docs/ACCEPTANCE_STANDARD_V0.1.md`。该文档定义证据等级、case 设计矩阵、缺陷级别、模块结论、真实用户硬门槛和回归规则，是验收方法的单一来源。

执行清单：`docs/UNIFIED_ACCEPTANCE_V0.1.md`。该清单当前状态为“已准备，未用户验收”；验收结论只能由用户明确确认后回填，不得因仓库测试或构建通过自动改为已验收。

范围边界：统一验收包是当前首批批次执行清单，验收标准中的 case 全集是 v0.1 发布前门槛。首批未覆盖的标准 case 保持“未执行”；首批批次通过不得被解释为模块全量验收通过或 v0.1 发布通过。

1. Agent 工作台：对话入口、模块导航、写入计划、确认与取消边界。
2. 能力资产：证据包导出、本地导入、证据状态和只读边界。
3. 简历管理：成品导入、简历对象列表、唯一草稿派生与保存、真实鼠标选区与右键拖拽（分别覆盖未滚动和滚动后的编辑器）、选区兜底 skill 的取消路径、建议生成 Trace、替换 Trace、定稿、当前投递版、导出标记和事实链状态。
4. 面试管理：JD 分析到公司机会、横向流程轨、节点抽屉、状态确认、产物挂载和文件桥。
5. 本地 Runtime：封闭 skill 注册表、dry-run、apply、备份、幂等与目标指纹。
6. Stage 6a：同步单元构建、校验、冲突分类和测试证据，只确认无联网与无写入边界。
7. Stage 6b：队列 list / enqueue / retry / cancel / mark / reconfirm、`gy --status` 摘要、备份和审计，只确认本地待发与无联网边界。
...
| 后端 P0 风险采用修复还是明确接受 | 影响 Stage 6c 启动时间 | 鉴权矩阵、Trace 脱敏语义和设备授权写入边界必须在 6c 设计前逐项给出结论 |
...
| 后端架构 | v0.1 保持模块化单体；架构基线是变更约束 | 避免拆服务和旧域回流扩大复杂度 |
| 实施节奏 | Stage 0 到 Stage 6 缓步推进 | 每阶段可验证、可回退 |
| 验收与实施门槛 | 仓库实现、测试和构建通过均不代表用户验收；进入 6c 前必须完成 6b 用户本地侧验收，并处理后端架构 P0 门槛 | 防止云端能力扩展压过体验阻断和数据边界问题 |

## 18. 文档修订记录

| 日期 | 修订 | 摘要 |
|---|---|---|
| 2026-09-01 | v0.1 初版 | 确立 Agent-first、网页大脑 / 本地工位、窄桥同步和大学生求职唯一目标 |
| 2026-09-02 | v0.1 r2 | 补充简历成品管理、公司机会流程树、防骗核查、产物挂载和显式文件桥 |
| 2026-09-03 | v0.1 r3 | 细化 Skill Runtime、简历事实链、三类冷启动、面试状态机与同步单元合同 |
| 2026-09-03 | v0.1 r4 | 校准 Stage 6a 完成边界，定义 6b 显式本地队列交付，拆分仓库验证与用户验收，并固定统一验收顺序 |
| 2026-09-03 | v0.1 r5 | 记录 Stage 6b 显式本地队列完成与仓库测试结果，更新统一验收顺序和 6c 前置门槛 |
| 2026-09-03 | v0.1 r6 | 简历管理改为以一份简历为唯一主对象，明确手工与 Agent 共享唯一草稿，并定义选区兜底 skill 的证据门槛与写权边界 |
| 2026-09-04 | v0.1 r7 | 校准当前实现、仓库验证与用户验收边界，细化简历对象和真实右键拖拽的人工验收项，并重申 6c 前置门槛；产品目标不变 |
| 2026-09-04 | v0.1 r8 | 纳入后端架构控制基线，明确模块化单体、云本地权威和后端 P0 门槛；6c 仍需用户本地侧验收与架构门槛双通过，产品目标不变 |
| 2026-09-04 | v0.1 r9 | 新增独立验收标准，固定证据等级、case 设计矩阵、缺陷分级、真实用户硬门槛与回归规则；产品目标和模块边界不变 |
| 2026-09-04 | v0.1 r10 | 新增模块大白话进度表，校准 A1-A7 机器预检、用户验收与后端阻断边界；产品目标、模块边界和 6c 门槛不变 |
| 2026-09-04 | v0.1 r11 | 登记后端 P0 加固批次：新增 85 条路由 / 23 个请求 Controller 的鉴权与所有权矩阵，Trace 新写入改为仅存脱敏摘要并提供 V23 历史数据重写；产品目标、模块边界、6b 用户验收门槛和“不得擅自启动 6c”不变 |
| 2026-09-04 | v0.1 r12 | 回填后端 P0 当前工程证据：49/49 回归通过，MariaDB 11.4 V22→V23 迁移、legacy 摘要化、哈希重算与敏感残留断言通过；明确 MySQL 8.4 与迁移后启动链路仍未验证，6c 门槛不变 |
