...
| 进行中 | 公司机会横向流程轨、节点抽屉、人工节点管理，以及本地 JD 分析合同 / 报告 | 防骗核查、公司机会写入、tracker 持久化、真实产物挂载和页面内 skill 执行未完成 |
...
以下清单是用户验收标准，不因仓库实现完成而自动勾选。当前可开始统一验收 Stage 1 到 Stage 4；Stage 5 只能验收公司机会流程轨 Demo 和本地 JD 分析报告，不能验收完整岗位评估到 tracker 的闭环；Stage 6 相关条目仍未开始实现。
...
| 岗位唯一识别如何处理同一公司多岗位 | 影响 tracker 去重 | 以公司、岗位、地点和招聘批次组合判断，在公司机会写入与 tracker 持久化前定稿 |
