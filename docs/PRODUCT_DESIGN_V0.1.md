...
当前实现边界：`cli/skill-runtime.mjs` v0.1 已落地 6 个封闭注册 skill、只读发现、计划校验、默认 dry-run、用户确认后的审批记录、幂等导入、同 `runId` 冲突保护和替换备份。它是 approval ledger：不调用 LLM，不执行 shell，不调用契约工具，也不写简历、能力资产、面试管理或 tracker 目标对象。契约 dispatcher、执行前后目标指纹和目标文件执行桥属于后续版本。
