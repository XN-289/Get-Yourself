...
| 项目 | 内容 |
|---|---|
| 文档状态 | Agent-first 实现校准修订版 r4，含 2026-09-03 实现进度、Stage 6a 本地确定性构建 / 校验状态、下一步 6b 边界与统一验收口径；待产品与技术评审与用户统一验收 |
| 产品阶段 | v0.1 最小闭环设计 |
...
| 顺序 | 事项 | 边界 |
|---|---|---|
| 1 | 继续实现 Stage 6b 显式本地队列 | 复用 6a 的确定性校验与状态规则；只保存用户确认过的摘要，不做后台上传 |
| 2 | 同步准备用户统一验收包 | 覆盖 Agent、能力资产、简历管理、面试管理、本地 Runtime 与 6a；验收与 6b 实现可并行 |
| 3 | 6b 通过仓库测试并完成用户本地侧验收后，才进入 6c 授权 API 设计 | 云端接口、数据库和网页投影不得提前实现 |
...
| 6b 显式本地队列 | 只保存用户确认过的摘要；提供用户可见的队列状态、取消、认证阻断、重绑阻断、冲突处理和用户触发重试 | 下一步实现。6a 仓库测试已通过，用户验收未完成；6b 仍不联网、不做自动重试或自动上传 |
