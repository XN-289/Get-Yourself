...
| Stage 5 | 进行中 | 公司机会流程轨、本地 JD 分析合同 / 报告、公司机会对象、tracker 持久化与节点 mutation 合同已落地；防骗核查闭环、前端持久化桥接、真实产物挂载和页面内 skill 执行未完成 |
...
| 进行中 | 公司机会横向流程轨、节点抽屉、人工节点管理，以及本地 JD 分析 / 公司机会 / tracker / 节点 mutation 合同 | 防骗核查、前端持久化桥接、真实产物挂载和页面内 skill 执行未完成 |
...
以下清单是用户验收标准，不因仓库实现完成而自动勾选。当前可开始统一验收 Stage 1 到 Stage 4；Stage 5 可开始验收公司机会流程轨 Demo、本地 JD 分析报告、公司机会到投递清单的本地写入和节点 mutation 合同，不能验收防骗核查、产物挂载、页面内 skill 或前端持久化闭环；Stage 6 相关条目仍未开始实现。
...
| 公司机会前端如何调用并确认本地节点 mutation | 影响 Stage 5 完整闭环 | 在真实产物挂载和页面内 skill 执行前，先定义前端展示、计划确认与本地执行结果回读的桥接 |
