...
| 项目 | 内容 |
|---|---|
| 文档状态 | 模块进度校准修订版 r10，含 2026-09-04 实现 / 机器预检 / 用户验收三层边界、简历对象化管理与选区兜底 skill 的人工验收口径、Stage 6a 与 6b 的仓库验证边界、统一验收顺序、后端架构 P0 门槛、6c 双门槛与独立验收标准；待产品与技术评审与用户统一验收 |
| 产品阶段 | v0.1 最小闭环设计 |
...
| 阶段 | 当前状态 | 说明 |
|---|---|---|
| Stage 0 | 文档已校准，待评审 | 产品、契约与后端架构控制基线已形成文档，仍需产品、技术评审确认 |
...
| 后端架构控制基线已建立、运行行为未变化 | Spring Boot 模块化单体、模块 / 路由归属、云本地权威、事务 outbox、AI / Trace 边界与架构风险清单已写入控制文档 | 该基线是变更约束，不是后端重构完成、安全风险修复完成或 Stage 6c 开始的证据 |
...
| 平台协同与后端 | 已有模块化单体架构控制、云本地权威边界和风险清单；设备绑定、证据导出等既有链路可验 | 架构基线已审查；后端 Maven 回归仍受本机缺 Maven 阻断，A0 只记录环境事实 | 后端 P0 风险逐项修复或明确接受；6c 设计前必须通过双门槛 |
...
| 3 | 完成后端架构 P0 门槛复核 | 鉴权矩阵、Trace 脱敏语义和未来设备授权写入边界必须被修复或由用户明确接受；架构文档本身不能替代处置 |
...
1. 数据承载接口的鉴权和用户所有权规则有明确矩阵，不再只依赖每个 Controller 自觉调用当前用户。
2. Trace 的脱敏标记必须反映真实脱敏结果；修复前不得写入简历全文、STAR 原文、凭据或原始个人文件。
3. 设备授权只能证明工位关系，不能变成第二个网页登录态；未来接收同步摘要必须有显式授权、幂等、冲突证据和 Trace。
...
截至 2026-09-04 15:08，A0 已完成环境基线记录，A1 到 A7 均已有一轮机器预检；但这些证据仍不改变任何用户验收结论。A0 中后端 Maven 测试因当前机器缺 Maven 继续如实阻断，不能被临时兼容环境或文档复核替代。
