...
4. 新增 `mq` 包，声明统一 Topic Exchange、三条 durable queue 和 routing key：
   - `get-yourself.event-index`：后续用于事件创建/更新后的 OpenSearch 索引同步。
   - `get-yourself.growth-tag`：后续用于完成活动/挑战后的成长标签抽取。
   - `get-yourself.user-profile`：后续用于用户画像刷新。
...
4. 新增 `EventSearchIndexMessageListener`，监听 `get-yourself.event-index` 队列。
...
5. 新增 `GrowthTagExtractionMessageListener`，监听 `get-yourself.growth-tag` 队列。
...
3. 后端缺少监听 `get-yourself.user-profile` 队列的消费者。
...
`docs/IRT_sys_agent_Scoring_System_v2.docx`
...
14. 新增 Outbox/RabbitMQ 链路：
    - 队列 `get-yourself.ability-evidence`
...
`docs/Evidence_Assessment_Agent_v2.md`
...
- `get-yourself.ability-evidence` 为持久化队列。
- 当前消息数为 0。
- 当前消费者数为 1。
